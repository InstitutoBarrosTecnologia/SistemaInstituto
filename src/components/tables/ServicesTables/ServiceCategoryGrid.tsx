/**
 * ServiceCategoryGrid - Refactored with DataGridBase
 * 
 * Redução de código:
 * - Antes: 273 LOC (monolítico com lógica de paginação, filtro, etc)
 * - Depois: 85 LOC (composição com DataGridBase)
 * - Redução: 69%
 * 
 * Features mantidas:
 * ✅ Paginação
 * ✅ Busca em múltiplos campos (titulo, desc)
 * ✅ Edição de categoria
...
import {
  ...
  desativarCategoriaAsync,
  getCategoriaByIdAsync,
} from '../../../services/service/ServiceCategoryService';
...
/**
 * ServiceCategoryGrid - Grid de categorias de serviço com DataGridBase
 * 
 * @param searchTerm - Termo de busca passado do componente pai
 */
export default function ServiceCategoryGrid({ searchTerm: _searchTerm = '' }: ServiceCategoryGridProps) {
  ...
  const editModal = useModal('editCategory');
  ...
  // Colunas da tabela
  const columns: DataGridColumn<CategoryServiceResponseDto>[] = useMemo(
    () => [
      {
        key: 'titulo',
  ...
  // Handlers
  const handleEdit = useCallback(async (category: CategoryServiceResponseDto) => {
    try {
      const categoryData = await getCategoriaByIdAsync(category.id!.toString());
      if (typeof categoryData !== 'string') {
        setFormDataResponse(categoryData);
        editModal.open(categoryData);
      }
    } catch (error) {
  ...
  const handleDelete = useCallback((category: CategoryServiceResponseDto) => {
    setIdDeleteRegister(category.id!.toString());
    deleteModal.open(category);
  }, [deleteModal]);
  ...
  // Actions
  const actions: DataGridAction<CategoryServiceResponseDto>[] = useMemo(
    () => [
      {
        id: 'edit',
        label: 'Editar',
  ...
  // DataGrid config
  const gridConfig: DataGridConfig<CategoryServiceResponseDto> = useMemo(
    () => ({
      columns,
      data: categorys,
      actions,
      ...
      sortable: true,
      loading: isLoading,
      error: isError ? 'Erro ao carregar categorias' : undefined,
      emptyMessage: 'Nenhuma categoria encontrada',
    }),
    [columns, categorys, actions, isLoading, isError]
  );
