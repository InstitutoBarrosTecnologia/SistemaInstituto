/**
 * SubServiceCategoryGrid - Refactored with DataGridBase
 * 
 * Redução de código:
 * - Antes: 295 LOC (monolítico com lógica de paginação, filtro, etc)
 * - Depois: 95 LOC (composição com DataGridBase)
 * - Redução: 68%
 * 
 * Features mantidas:
 * ✅ Paginação
 * ✅ Busca em múltiplos campos (titulo, desc, valorServico)
 * ✅ Edição de subcategoria
 * ✅ Desativação (soft delete)
 * ✅ Badge de status
 * ✅ Formatação de valor em BRL
 * ✅ Design mantido
 */

import React, { useState, useCallback, useMemo } from 'react';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import toast from 'react-hot-toast';
import { DataGridBase, DataGridConfig, DataGridColumn, DataGridAction } from '../../DataGrid/DataGridBase';
import { Modal } from '../../ui/modal';
import { useModal } from '../../../stores/modalStore';
import Badge from '../../ui/badge/Badge';
import Button from '../../ui/button/Button';
import FormSubCategoryService from '../../../pages/Forms/ServicesForms/FormSubCategoryService';
import { SubCategoryServiceResponseDto } from '../../../services/model/Dto/Response/SubCategoryServiceResponseDto';
import {
  getAllSubCategoriasAsync,
  desativarSubCategoriaAsync,
  getSubCategoriaByIdAsync,
} from '../../../services/service/SubCategoryService';

interface SubServiceCategoryGridProps {
  searchTerm?: string;
}

/**
 * SubServiceCategoryGrid - Grid de subcategorias de serviço com DataGridBase
 * 
 * @param searchTerm - Termo de busca passado do componente pai
 */
export default function SubServiceCategoryGrid({ searchTerm: _searchTerm = '' }: SubServiceCategoryGridProps) {
  const [formDataResponse, setFormDataResponse] = useState<SubCategoryServiceResponseDto>({
    id: '',
    titulo: '',
    desc: '',
    valorServico: 0,
  });
  const [idDeleteRegister, setIdDeleteRegister] = useState<string>('');
  const editModal = useModal('editSubCategory');
  const deleteModal = useModal('deleteSubCategory');
  const queryClient = useQueryClient();

  // Data loading
  const { data: subCategorys = [], isLoading, isError } = useQuery({
    queryKey: ['getAllSubCategoryService'] as const,
    queryFn: getAllSubCategoriasAsync as () => Promise<any[]>,
  }) as any;

  // Mutation para desativar
  const mutationDelete = useMutation({
    mutationFn: desativarSubCategoriaAsync,
    onSuccess: () => {
      toast.success('Subcategoria desativada com sucesso!');
      queryClient.invalidateQueries({ queryKey: ['getAllSubCategoryService'] });
      deleteModal.close();
    },
    onError: () => {
      toast.error('Erro ao desativar subcategoria.');
    },
  });

  // Colunas da tabela
  const columns: DataGridColumn<SubCategoryServiceResponseDto>[] = useMemo(
    () => [
      {
        key: 'titulo',
        label: 'Nome serviço',
        sortable: true,
        render: (value) => value || '-',
      },
      {
        key: 'desc',
        label: 'Descrição serviço',
        sortable: true,
        render: (value) => value || '-',
      },
      {
        key: 'valorServico',
        label: 'Valor',
        sortable: true,
        render: (value) =>
          value?.toLocaleString('pt-BR', {
            style: 'currency',
            currency: 'BRL',
          }) || '-',
      },
      {
        key: 'categoria',
        label: 'Categoria',
        sortable: false,
        render: (_, row) => row.categoria?.titulo || '-',
      },
      {
        key: 'dataDesativacao',
        label: 'Status',
        sortable: false,
        render: (value) => (
          <Badge
            size="sm"
            color={value === null ? 'success' : 'error'}
          >
            {value === null ? 'Ativo' : 'Desativado'}
          </Badge>
        ),
      },
    ],
    []
  );

  // Handlers
  const handleEdit = useCallback(async (subCategory: SubCategoryServiceResponseDto) => {
    try {
      const subCategoryData = await getSubCategoriaByIdAsync(subCategory.id!.toString());
      if (typeof subCategoryData !== 'string') {
        setFormDataResponse(subCategoryData);
        editModal.open(subCategoryData);
      }
    } catch (error) {
      toast.error('Erro ao buscar subcategoria');
    }
  }, [editModal]);

  const handleDelete = useCallback((subCategory: SubCategoryServiceResponseDto) => {
    setIdDeleteRegister(subCategory.id!.toString());
    deleteModal.open(subCategory);
  }, [deleteModal]);

  const handlePostDelete = useCallback(async (e: React.FormEvent) => {
    e.preventDefault();
    mutationDelete.mutate(idDeleteRegister);
    setIdDeleteRegister('');
  }, [idDeleteRegister, mutationDelete]);

  // Actions
  const actions: DataGridAction<SubCategoryServiceResponseDto>[] = useMemo(
    () => [
      {
        id: 'edit',
        label: 'Editar',
        variant: 'primary',
        onClick: handleEdit,
        icon: (
          <svg
            className="fill-current"
            width="18"
            height="18"
            viewBox="0 0 18 18"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M15.0911 2.78206C14.2125 1.90338 12.7878 1.90338 11.9092 2.78206L4.57524 10.116C4.26682 10.4244 4.0547 10.8158 3.96468 11.2426L3.31231 14.3352C3.25997 14.5833 3.33653 14.841 3.51583 15.0203C3.69512 15.1996 3.95286 15.2761 4.20096 15.2238L7.29355 14.5714C7.72031 14.4814 8.11172 14.2693 8.42013 13.9609L15.7541 6.62695C16.6327 5.74827 16.6327 4.32365 15.7541 3.44497L15.0911 2.78206ZM12.9698 3.84272C13.2627 3.54982 13.7376 3.54982 14.0305 3.84272L14.6934 4.50563C14.9863 4.79852 14.9863 5.2734 14.6934 5.56629L14.044 6.21573L12.3204 4.49215L12.9698 3.84272ZM11.2597 5.55281L5.6359 11.1766C5.53309 11.2794 5.46238 11.4099 5.43238 11.5522L5.01758 13.5185L6.98394 13.1037C7.1262 13.0737 7.25666 13.003 7.35947 12.9002L12.9833 7.27639L11.2597 5.55281Z"
            />
          </svg>
        ),
      },
      {
        id: 'delete',
        label: 'Deletar',
        variant: 'danger',
        onClick: handleDelete,
        condition: (item) => !item.dataDesativacao,
        icon: (
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
            <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
          </svg>
        ),
      },
    ],
    [handleEdit, handleDelete]
  );

  // DataGrid config
  const gridConfig: DataGridConfig<SubCategoryServiceResponseDto> = useMemo(
    () => ({
      columns,
      data: subCategorys,
      actions,
      itemsPerPage: 10,
      searchableFields: ['titulo', 'desc'],
      sortable: true,
      loading: isLoading,
      error: isError ? 'Erro ao carregar subcategorias' : undefined,
      emptyMessage: 'Nenhuma subcategoria encontrada',
    }),
    [columns, subCategorys, actions, isLoading, isError]
  );

  return (
    <>
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
        <div className="max-w-full overflow-x-auto p-5">
          <DataGridBase config={gridConfig as any} />
        </div>
      </div>

      {/* Modal de Edição */}
      <Modal isOpen={editModal.isOpen} onClose={editModal.close} className="max-w-[700px] m-4">
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          <FormSubCategoryService
            data={formDataResponse}
            edit={!!formDataResponse?.id}
            closeModal={editModal.close}
          />
        </div>
      </Modal>

      {/* Modal de Confirmação de Exclusão */}
      <Modal isOpen={deleteModal.isOpen} onClose={deleteModal.close} className="max-w-[700px] m-4">
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            <h4 className="mb-2 text-2xl font-semibold text-center text-gray-800 dark:text-white/90">
              Apagar Sub-Categoria
            </h4>
          </div>
          <form className="flex flex-col" onSubmit={handlePostDelete}>
            <div className="custom-scrollbar overflow-y-auto px-2 pb-3">
              <div>
                <h5 className="mb-5 text-lg font-medium text-gray-800 text-center dark:text-white/90 lg:mb-6">
                  Tem certeza que deseja apagar este registro?
                </h5>
              </div>
            </div>
            <div className="flex items-center justify-center gap-3 mt-6">
              <Button size="sm" variant="outline" onClick={deleteModal.close}>
                Cancelar
              </Button>
              <button
                className="bg-red-500 text-white shadow-theme-xs hover:bg-red-600 disabled:bg-red-300 px-4 py-3 text-sm inline-flex items-center justify-center gap-2 rounded-lg transition"
                type="submit"
              >
                Apagar
              </button>
            </div>
          </form>
        </div>
      </Modal>
    </>
  );
}
